...
available_memory=`echo $mem_info | cut -c14-26 | xargs`
...
then
    #XDG_RUNTIME_DIR=/run/user/$(id -u) notify-send -u normal "Low memory" "only $(echo $available_memory | tr -s " ") GB available!"
